...
RUN apt-get update \
    && apt-get install -yq libmpc-dev libmpfr-dev libgmp-dev python2.7-dev libboost-program-options-dev \
    && apt-get clean && rm -rf /var/lib/apt/lists/* /tmp/* \
    && ln -s /usr/lib/x86_64-linux-gnu/libmpfr.so.6.0.1 /usr/lib/x86_64-linux-gnu/libmpfr.so.4 \
    && ln -s /usr/lib/x86_64-linux-gnu/libboost_program_options.so /usr/lib/x86_64-linux-gnu/libboost_program_options.so.1.54.0
